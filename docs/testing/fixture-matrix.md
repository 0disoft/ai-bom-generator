...
| `poetry-lock-corpus` | Separate publication-safe Poetry 2.0 and 2.1 TOML fixtures preserve the category-era and group-era package shapes observed in Poetry's versioned locker implementation. The corpus covers registry, legacy index, Git, and direct URL sources, resolved revisions, file hashes, equal group markers, and conflicting group markers without claiming group selection. Covered by focused parser tests and required in the source distribution. |
| `pipenv-lock` | Publication-safe Pipenv `Pipfile.lock` specification 6 JSON covers default/develop groups, exact registry pins, source-name resolution, hashes, extras, explicit and shorthand PEP 508 markers, Git references, remote files, and editable local paths. Duplicate keys, unsupported specifications, ambiguous sources, malformed evidence, and parser budgets are covered without claiming group selection. Seeded property tests exercise 256 JSON shapes plus 120 generated evidence combinations without allowing unexpected exceptions or fabricated versions, sources, markers, or hashes. Required in the source distribution. |
| `pipenv-lock-corpus` | Lockfiles generated from the same bounded public input by exact Pipenv 2023.12.1 and 2026.6.2 releases prove specification 6 registry compatibility across producer generations. Tests preserve exact versions, default/develop organization, markers, public index resolution, hashes, and equivalent normalized parser output. Both lockfiles and their provenance README are required in the source distribution. |
