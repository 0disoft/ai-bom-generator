...
| `DEPENDENCY_PARSE_FAILED` | Supported dependency file is unreadable or invalid as its declared UTF-8, JSON, TOML, or bounded YAML format. | CLI invalid uv.lock plus Poetry, Pipenv, and conda-lock parser tests |
| `DEPENDENCY_PARSE_PARTIAL` | Supported dependency file contains skipped directives or malformed entries while retaining valid packages. | CLI malformed requirements plus Poetry, Pipenv, and conda-lock parser tests |
