...
- Dependency files declared explicitly in config. Bounded parsers read
  `uv.lock`, requirements files, and unified conda-lock v1 YAML into normalized
  package evidence while preserving the original file reference. Automatic
...
- YAML parser dependency: `pyyaml>=6.0.3,<7` is approved for bounded conda-lock
  v1 parsing through a restricted safe loader.
...
- Lockfile support set: explicit config-declared dependency file references,
  with bounded local parsing for `uv.lock`, requirements files, and unified
  conda-lock v1 YAML. Parsing may be disabled per reference with `parse = false`.
